...
void hybrid_kernel_III(VECTOR_TYPE* res, SETUP_TYPE* setup, VECTOR_TYPE* invec, int spmvmOptions){
...
#pragma omp for
		...
		if (tid == setup->communicator->threads-1){ /* Kommunikations-thread */
			/***********************************************************************
			...
			//MPI_safecall(MPI_Waitall(setup->communicator->nodes, send_request, send_status));
			...
			if( tid == setup->communicator->threads-2 ) {
