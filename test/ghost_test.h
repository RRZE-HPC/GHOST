        ...
        if (t & GHOST_DT_REAL) {\
            if (t & GHOST_DT_DOUBLE) {\
                ...
                if (return_if_differ_err > D_EPS*f) {\
                    GHOST_ERROR_LOG("Values differ: error %8.4e > %8.4e", return_if_differ_err, D_EPS*f);\
                    return EXIT_FAILURE;\
                }\
            } else {\
                float return_if_differ_err = fabsf(*((float *)a) - *((float *)b));\
                if (return_if_differ_err > S_EPS*f) {\
                    GHOST_ERROR_LOG("Values differ: error %8.4e > %8.4e", return_if_differ_err, S_EPS*f);\
                    return EXIT_FAILURE;\
                }\
            }\
        } else {\
            if (t & GHOST_DT_DOUBLE) {\
                ...
                if (return_if_differ_err > D_EPS*f) {\
                    GHOST_ERROR_LOG("Values differ: real part error %8.4e > %8.4e", return_if_differ_err, D_EPS*f);\
                    return EXIT_FAILURE;\
                }\
                return_if_differ_err = fabs(imag(*(std::complex<double> *)a) - imag(*(std::complex<double> *)b));\
                if (return_if_differ_err > D_EPS*f) {\
                    GHOST_ERROR_LOG("Values differ: imag part error %8.4e > %8.4e", return_if_differ_err, D_EPS*f);\
                    return EXIT_FAILURE;\
                }\
            } else {\
