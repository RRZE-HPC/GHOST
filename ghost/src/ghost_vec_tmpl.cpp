...
template <typename v_t> void ghost_vec_fromRand_tmpl(ghost_vec_t *vec, ghost_context_t * ctx)
{
	...
	vec->val = allocateMemory(vec->traits->nvecs*vec->traits->nrows*ghost_sizeofDataType(vec->traits->datatype),"vec->val");
	int i;

#pragma omp parallel for schedule(runtime)
	for (i=0; i<vec->traits->nrows; i++) {
		((v_t *)(vec->val))[i] = (v_t)(rand()*1./RAND_MAX + I*rand()*1./RAND_MAX);
	}
}

template<typename v_t> int ghost_vecEquals_tmpl(ghost_vec_t *a, ghost_vec_t *b)
{
	double tol = 1e-5; // TODO
	int i;
	for (i=0; i<a->traits->nrows; i++) {
			if (fabs(creal((complex double)VAL(a,i) - (complex double)VAL(b,i))) > tol ||
					fabs(cimag((complex double)VAL(a,i) - (complex double)VAL(b,i))) > tol)
				return 0;
	}
	return 1;
}
...
{ return ghost_normalizeVector_tmpl<complex double>(vec); }

extern "C" void c_ghost_normalizeVector(ghost_vec_t *vec) 
{ return ghost_normalizeVector_tmpl<complex float>(vec); }
...
{ return ghost_vec_dotprod_tmpl<complex double>(vec,vec2,res); }

extern "C" void c_ghost_vec_dotprod(ghost_vec_t *vec, ghost_vec_t *vec2, void *res) 
{ return ghost_vec_dotprod_tmpl<complex float>(vec,vec2,res); }
...
{ return ghost_vec_scale_tmpl<complex double>(vec, scale); }

extern "C" void c_ghost_vec_scale(ghost_vec_t *vec, void *scale) 
{ return ghost_vec_scale_tmpl<complex float>(vec, scale); }
...
{ return ghost_vec_axpy_tmpl<complex double>(vec, vec2, scale); }

extern "C" void c_ghost_vec_axpy(ghost_vec_t *vec, ghost_vec_t *vec2, void *scale) 
{ return ghost_vec_axpy_tmpl<complex float>(vec, vec2, scale); }
...
{ return ghost_vec_fromRand_tmpl<complex double>(vec,ctx); }

extern "C" void c_ghost_vec_fromRand(ghost_vec_t *vec, ghost_context_t *ctx) 
{ return ghost_vec_fromRand_tmpl<complex float>(vec,ctx); }
...
{ return ghost_vecEquals_tmpl<complex double>(vec,vec2); }

extern "C" int c_ghost_vecEquals(ghost_vec_t *vec, ghost_vec_t *vec2) 
{ return ghost_vecEquals_tmpl<complex float>(vec,vec2); }
