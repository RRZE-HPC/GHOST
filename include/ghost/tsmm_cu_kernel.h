...
bool tsmm_fallback(const int K, const T alpha, const T *A, const int lda,
...
template <typename T, int M, int N>
bool tsmm_v1(const int K, const T alpha, const T *A, const int lda, const T *B,
...
template <typename T, int M, int N>
bool tsmm_v2(const int K, const T alpha, const T *A, const int lda, const T *B,
             const int ldb, const T beta, T *C, const int ldc) {
  if (M >= 32 / sizeof(T)) {
    const int threadsPerBlock = 256;
    int deviceUsed;
...
template <typename T, int M, int N>
bool tsmm_cublas(const int K, const T alpha, const T *A, const int lda,
...
template <typename T, int M, int N>
bool ghost_tsmm_cu_rm_cm(T *C, const T *A, const T *B, const T alpha,
                         const T beta, const ghost_lidx K, const ghost_lidx ldc,
                         const ghost_lidx lda, const ghost_lidx ldb) {
  if (M >= 7 && N >= 4 &&
      tsmm_v1<T, M, N>(K, alpha, A, lda, B, ldb, beta, C, ldc))
    return true;
  if (M >= 14 && N >= 14 &&
      tsmm_cublas<T, M, N>(K, alpha, A, lda, B, ldb, beta, C, ldc))
    return true;
  if (M >= 7 && N <= 5 &&
      tsmm_v2<T, M, N>(K, alpha, A, lda, B, ldb, beta, C, ldc))
    return true;
