...
#define LOOP_pre(start,end,MT,VT) \
...
inline void KACZ_pre_Kernel(int start, int end, void *args)
{
    ...
    MT *rnm_sq = (MT *)(kaczArg->mat->rowNormSq);
    MT omega = *((MT*)(kaczArg->omega));
    ghost_sparsemat *mat = kaczArg->mat;

    LOOP_pre(start, end, MT, VT);
}
...
void ghost_kacz_pre_RACE(ghost_densemat *b, ghost_sparsemat *mat, ghost_densemat *x, int iterations)
{
    ...
    int kaczId = ce->registerFunction(&KACZ_pre_Kernel, argPtr);
