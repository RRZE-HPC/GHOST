...
namespace std
{
    template<> struct hash<ghost_gs_RACE_parameters>
    {
        typedef ghost_gs_RACE_parameters argument_type;
...
static bool operator==(const ghost_gs_RACE_parameters& a, const ghost_gs_RACE_parameters& b)
{
...
static std::unordered_map<ghost_gs_RACE_parameters, ghost_gs_RACE_kernel>
ghost_gs_RACE_kernels = std::unordered_map<ghost_gs_RACE_parameters,ghost_gs_RACE_kernel>();


extern "C" ghost_error ghost_gs_RACE(ghost_densemat *lhs, ghost_sparsemat *mat, ghost_densemat *rhs, int iterations)
{
...
//    ghost_gs_RACE_parameters checkPars;

    // if map is empty include generated code for map construction
    if (ghost_gs_RACE_kernels.empty()) {
#include "sell_gs_RACE_avx.def"
#include "sell_gs_RACE_mic.def"
   }

    ghost_gs_RACE_kernel kernel = NULL;
    ghost_gs_RACE_parameters p;
    ...
    if ((lhs->traits.flags & GHOST_DENSEMAT_SCATTERED) ||
        ...
        GHOST_ERROR_LOG("SCATTERED VIEWS NOT IMPLEMENTED");
            ...
            for (pos_storage = 0; pos_storage < n_storage; pos_storage++) {
                ...
                for (std::vector<ghost_implementation>::iterator impl = try_impl.begin(); impl != try_impl.end(); impl++) {
                    ...
                    for (p.alignment = opt_align; (int)p.alignment >= GHOST_UNALIGNED; p.alignment = (ghost_alignment)((int)p.alignment-1)) {
                        ...
                        GHOST_INFO_LOG("Try chunkheight=%s, blocksz=%s, impl=%s, storage=%s, %s",
                        ...
                        kernel = ghost_gs_RACE_kernels[p];
...
end_of_loop:
    ...
    ghost_autogen_string_add("GS",oss.str().c_str());
    ...
    if (kernel) {
        ...
        ret = kernel(lhs,mat,rhs,iterations);
           ...
           GHOST_PERFWARNING_LOG("Using fallback kernel");
           ghost_gs_RACE_fallback(lhs,mat,rhs,iterations);
    }
