...
ghost_error ghost_map_create(ghost_map **map, ghost_gidx gnrows, ghost_mpi_comm comm)
{
    ...
    GHOST_CALL_GOTO(ghost_malloc((void **)&((*map)->lnrows),sizeof(ghost_lidx)*nranks),err,ret);
...
ghost_error ghost_map_create_distribution(ghost_map *map, ghost_sparsemat_src_rowfunc *matsrc, ghost_mpi_comm mpicomm, double weight, ghost_map_dist_type distType)
{
    ...
    { // read rpt and fill lfrow, lnrows, lfent, lnents
        ...
        if (me == 0) {
            GHOST_CALL_GOTO(ghost_malloc((void **)&rpt,sizeof(ghost_gidx)*(map->gnrows+1)),err,ret);
#pragma omp parallel for schedule(runtime)
            for( row = 0; row < map->gnrows+1; row++ ) {
                rpt[row] = 0;
            }
            char *tmpval = NULL;
            ...
            for (i=0; i<nranks-1; i++){
                map->lnrows[i] = map->goffs[i+1] - map->goffs[i] ;
                //(*context)->lnEnts[i] = (*context)->lfEnt[i+1] - (*context)->lfEnt[i] ;
            }

            map->lnrows[nranks-1] = map->gnrows - map->goffs[nranks-1] ;
        ...
        MPI_CALL_GOTO(MPI_Bcast(map->lnrows, nranks, ghost_mpi_dt_lidx, 0, mpicomm),err,ret);
    ...
    { // don't read rpt, only fill lfrow, lnrows, rest will be done after some matrix from*() function
        ...
        ghost_lidx my_target_rows = (ghost_lidx)(map->gnrows*((double)weight/(double)allweights));
        if (my_target_rows == 0) {
        ...
        for (i=0; i<nranks-1; i++){
            ...
            map->lnrows[i] = (ghost_lidx)lnrows;
        }
        ghost_gidx lnrows = map->gnrows - map->goffs[nranks-1];
        if (lnrows > (ghost_gidx)GHOST_LIDX_MAX) {
        ...
        map->lnrows[nranks-1] = (ghost_lidx)lnrows;
...
void ghost_map_destroy(ghost_map *map)
{
        ...
        free(map->lnrows); map->lnrows = NULL;
