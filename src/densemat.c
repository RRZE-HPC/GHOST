...
const ghost_densemat_traits GHOST_DENSEMAT_TRAITS_INITIALIZER = {
    ...
    .compute_with = GHOST_IMPLEMENTATION_DEFAULT,
  ...
  */
    ...
    /*
       if(ctx==NULL || ctx->perm_local == NULL) {
           ...
           GHOST_CALL_GOTO(ghost_malloc((void
       **)&((*vec)->perm_local),sizeof(ghost_densemat_permutation)),err,ret);
        ...
        if(ctx==NULL || ctx->perm_global == NULL) {
                    ...
                    GHOST_CALL_GOTO(ghost_malloc((void
       **)&((*vec)->perm_global),sizeof(ghost_densemat_permutation)),err,ret);
    ...
    MPI_CALL_RETURN(MPI_Type_vector(
        (*vec)->nblock, (*vec)->blocklen, (*vec)->stride, (*vec)->mpidt, &((*vec)->fullmpidt)));
    ...
    GHOST_CALL_GOTO(
        ghost_malloc((void **)&((*vec)->subdm), sizeof(ghost_densemat *) * (*vec)->nsub), err, ret);
    if ((*vec)->nsub == 1) {
        (*vec)->subdm[0] = *vec;
        (*vec)->coloff = 0;
    } else {
...
ghost_error ghost_densemat_info_string(char **str, ghost_densemat *densemat)
{
    ...
    ghost_line_string(
        str, "Dimension", NULL, "%" PRLIDX "x%" PRLIDX, DM_NROWS(densemat), densemat->traits.ncols);
    ghost_line_string(str, "Padded dimension", NULL, "%" PRLIDX "x%" PRLIDX, DM_NROWSPAD(densemat),
        densemat->traits.ncolspadded);
    ...
    ghost_line_string(
        str, "View", NULL, "%s", densemat->traits.flags & GHOST_DENSEMAT_VIEW ? "Yes" : "No");
    ghost_line_string(str, "Scattered", NULL, "%s",
        densemat->traits.flags & GHOST_DENSEMAT_SCATTERED ? "Yes" : "No");
    if (densemat->traits.flags & GHOST_DENSEMAT_VIEW) {
        ghost_line_string(str, "Dimension of viewed densemat", NULL, "%" PRLIDX "x%" PRLIDX,
            DM_NROWS(densemat->src), densemat->src->traits.ncols);
    ...
    ghost_line_string(
        str, "Storage order", NULL, "%s", ghost_densemat_storage_string(densemat->traits.storage));
...
ghost_error ghost_densemat_halocommInit_common(
    ghost_densemat *vec, ghost_context *ctx, ghost_densemat_halo_comm *comm)
{
#ifdef GHOST_HAVE_MPI
    ...
    if (!comm->wishptr) {
        GHOST_CALL_GOTO(
            ghost_malloc((void **)&comm->wishptr, (nprocs + 1) * sizeof(ghost_lidx)), err, ret);
    ...
    if (!comm->request) {
        GHOST_CALL_GOTO(
            ghost_malloc((void **)&comm->request, sizeof(MPI_Request) * nMsgsOverall), err, ret);
    }
    if (!comm->status) {
        GHOST_CALL_GOTO(
            ghost_malloc((void **)&comm->status, sizeof(MPI_Status) * nMsgsOverall), err, ret);
    }
...
ghost_error ghost_densemat_halocomm_start(
    ghost_densemat *vec, ghost_context *ctx, ghost_densemat_halo_comm *comm)
{
#ifdef GHOST_HAVE_MPI
    GHOST_FUNC_ENTER(GHOST_FUNCTYPE_COMMUNICATION)
    ...
    for (from = 0; from < ctx->nwishpartners; from++) {
        ...
        ghost_datatransfer_register("spmv_halo", GHOST_DATATRANSFER_IN, from_PE,
            ctx->wishes[from_PE] * vec->elSize * vec->traits.ncols);
#endif
        int msg;
        ...
        for (msg = 0; msg < nmsgs - 1; msg++) {
            MPI_CALL_GOTO(MPI_Irecv(recv + msg * msgSizeRows * rowsize, msgSizeEls, vec->mpidt,
                              from_PE, from_PE, ctx->mpicomm, &comm->request[comm->msgcount]),
                err, ret);
            comm->msgcount++;
        }

        // remainder
        MPI_CALL_GOTO(MPI_Irecv(recv + msg * msgSizeRows * rowsize,
                          ctx->wishes[from_PE] * vec->traits.ncols - msg * msgSizeEls, vec->mpidt,
                          from_PE, from_PE, ctx->mpicomm, &comm->request[comm->msgcount]),
            err, ret);
        comm->msgcount++;
    }
    for (to = 0; to < ctx->nduepartners; to++) {
...
#ifdef GHOST_TRACK_DATATRANSFERS
        ghost_datatransfer_register("spmv_halo", GHOST_DATATRANSFER_OUT, to_PE,
            ctx->dues[to_PE] * vec->elSize * vec->traits.ncols);
#endif
        int msg;
        ...
        for (msg = 0; msg < nmsgs - 1; msg++) {
            MPI_CALL_GOTO(MPI_Isend(workbuf + comm->dueptr[to_PE] * vec->elSize * vec->traits.ncols
                                  + msg * msgSizeRows * rowsize,
                              msgSizeEls, vec->mpidt, to_PE, me, ctx->mpicomm,
                              &comm->request[comm->msgcount]),
                err, ret);
            comm->msgcount++;
        }
        ...
        MPI_CALL_GOTO(MPI_Isend(workbuf + comm->dueptr[to_PE] * vec->elSize * vec->traits.ncols
                              + msg * msgSizeRows * rowsize,
                          ctx->dues[to_PE] * vec->traits.ncols - msg * msgSizeEls, vec->mpidt,
                          to_PE, me, ctx->mpicomm, &comm->request[comm->msgcount]),
            err, ret);
        comm->msgcount++;
    }
...
ghost_error ghost_densemat_init_val(ghost_densemat *x, void *val)
{
    ...
    for (ghost_lidx s = 0; s < x->nsub; s++) {
        SELECT_BLAS1_KERNEL(kernels, x->traits.location, x->traits.compute_at, x->traits.storage,
            ret, x->subdm[s], val);
    }
...
ghost_error ghost_densemat_halocomm_init(
    ghost_densemat *vec, ghost_context *ctx, ghost_densemat_halo_comm *comm)
{
    CALL_DENSEMAT_FUNC(vec, halocommInit, vec, ctx, comm);
}

ghost_error ghost_densemat_halocomm_finalize(
    ghost_densemat *vec, ghost_context *ctx, ghost_densemat_halo_comm *comm)
{
    CALL_DENSEMAT_FUNC(vec, halocommFinalize, vec, ctx, comm);
}

ghost_error ghost_densemat_download(ghost_densemat *vec) { CALL_DENSEMAT_FUNC(vec, download, vec); }

ghost_error ghost_densemat_upload(ghost_densemat *vec) { CALL_DENSEMAT_FUNC(vec, upload, vec); }
...
ghost_error ghost_densemat_create_and_view_densemat(ghost_densemat **x, ghost_densemat *src,
    ghost_lidx nr, ghost_lidx roffs, ghost_lidx nc, ghost_lidx coffs)
{
...
ghost_error ghost_densemat_create_and_view_densemat_scattered(ghost_densemat **x,
    ghost_densemat *src, ghost_lidx nr, ghost_lidx *ridx, ghost_lidx nc, ghost_lidx *cidx)
{
    CALL_DENSEMAT_FUNC(src, viewScatteredVec, src, x, nr, ridx, nc, cidx);
}

ghost_error ghost_densemat_create_and_view_densemat_cols(
    ghost_densemat **x, ghost_densemat *src, ghost_lidx nc, ghost_lidx coffs)
{
    CALL_DENSEMAT_FUNC(src, viewCols, src, x, nc, coffs);
}

ghost_error ghost_densemat_create_and_view_densemat_cols_scattered(
    ghost_densemat **x, ghost_densemat *src, ghost_lidx nc, ghost_lidx *cidx)
{
...
ghost_error ghost_densemat_init_densemat(
    ghost_densemat *x, ghost_densemat *y, ghost_lidx roffs, ghost_lidx coffs)
{
...
ghost_error ghost_densemat_compress(ghost_densemat *vec) { CALL_DENSEMAT_FUNC(vec, compress, vec); }
...
ghost_error ghost_densemat_distribute(
    ghost_densemat *vec, ghost_densemat *localVec, ghost_context *ctx)
{
    CALL_DENSEMAT_FUNC(vec, distributeVector, vec, localVec, ctx);
}

ghost_error ghost_densemat_clone(
    ghost_densemat **dst, ghost_densemat *src, ghost_lidx nc, ghost_lidx coffs)
{
