...
//      int nthread[1];
      ...
      int *nthread = (int*)malloc(1*sizeof(int));
...
*/
  ...
  // mat->kacz_setting.active_threads = nthread[0]; 
   printf("active threads = %d\n", mat->kacz_setting.active_threads);   
 ...
 //  GHOST_CALL_GOTO(ghost_malloc((void **)&new_zone_ptr,sizeof(ghost_lidx)*(4*n_zones+2)),err,ret);
   ...
   ghost_lidx total_bw = lower_bw + upper_bw;

   double diagonal_slope = (double)(mat->nrows)/mat->ncols;
