...
#define LOOP_fission(start,end,MT,VT) \
...
inline void KACZ_fission_Kernel(int start, int end, void *args)
{
...
void ghost_kacz_fission_RACE(ghost_densemat *b, ghost_sparsemat *mat, ghost_densemat *x, int iterations)
{
    ...
    int kaczId = ce->registerFunction(&KACZ_fission_Kernel, argPtr);
