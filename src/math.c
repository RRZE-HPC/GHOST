...
static void ghost_mpi_add_densemat_d(void *in, void *inout, int *len, MPI_Datatype *dtype)
{
    ...
    for (int i = 0; i < count; i++) {
        for (int j = 0; j < blocklen; j++) { inoutvec[i * stride + j] += invec[i * stride + j]; }
    }
}
...
ghost_error ghost_mpi_operations_create()
{
    ...
    MPI_CALL_RETURN(
        MPI_Op_create((MPI_User_function *)&ghost_mpi_add_densemat_d, 1, &GHOST_MPI_OP_SUM_DENSEMAT_D));
    MPI_CALL_RETURN(
        MPI_Op_create((MPI_User_function *)&ghost_mpi_add_densemat_s, 1, &GHOST_MPI_OP_SUM_DENSEMAT_S));
    MPI_CALL_RETURN(
        MPI_Op_create((MPI_User_function *)&ghost_mpi_add_densemat_c, 1, &GHOST_MPI_OP_SUM_DENSEMAT_C));
    MPI_CALL_RETURN(
        MPI_Op_create((MPI_User_function *)&ghost_mpi_add_densemat_z, 1, &GHOST_MPI_OP_SUM_DENSEMAT_Z));
#else
...
ghost_error ghost_spmv_nflops(int *nFlops, ghost_datatype m_t, ghost_datatype v_t)
{
    ...
    *nFlops = 2;
    ...
    if (m_t & GHOST_DT_COMPLEX) {
        if (v_t & GHOST_DT_COMPLEX) { *nFlops = 8; }
    } else {
        if (v_t & GHOST_DT_COMPLEX) { *nFlops = 4; }
    }
...
int ghost_spmtv_perf(double *perf, double time, void *varg)
{
    GHOST_FUNC_ENTER(GHOST_FUNCTYPE_UTIL|GHOST_FUNCTYPE_BENCH);

    UNUSED(time);
    ghost_spmtv_perf_args arg = *(ghost_spmtv_perf_args *)varg;
    UNUSED(arg);
    (*perf) = 2.0*(arg.globalnnz*1e-9)/(time);
...
int ghost_spmv_perf(double *perf, double time, void *varg)
{
    ...
    if (!(arg.flags & GHOST_SPMV_AXPY)
        && !(arg.flags & GHOST_SPMV_AXPBY)) { // y=Ax case, we have one ADD less in each row as tmp=0 before the inner loop
        flops -= flopsPerAdd * nrow * ncol;
    }
    if (arg.flags & GHOST_SPMV_AXPBY) { flops += flopsPerMul * nrow * ncol; }
    if (arg.flags & GHOST_SPMV_SHIFT) {
        flops += (flopsPerMul + flopsPerAdd) * nrow * ncol;
    } else if (arg.flags & GHOST_SPMV_VSHIFT) {
        flops += (flopsPerMul + flopsPerAdd) * nrow * ncol;
    }
    if (arg.flags & GHOST_SPMV_SCALE) { flops += flopsPerMul * nrow * ncol; }
    if (arg.flags & GHOST_SPMV_DOT_YY) { flops += (flopsPerMulSame + 1) * nrow * ncol; }
    if (arg.flags & GHOST_SPMV_DOT_XY) { flops += (flopsPerMul + flopsPerAdd) * nrow * ncol; }
    if (arg.flags & GHOST_SPMV_DOT_XX) { flops += (flopsPerMulSame + 1) * nrow * ncol; }
...
ghost_error ghost_axpbypcz(
    ghost_densemat *y, ghost_densemat *x, void *a, void *b, ghost_densemat *z, void *c)
{
    ...
    typedef ghost_error (*ghost_axpbypcz_kernel)(
        ghost_densemat *, ghost_densemat *, void *, void *, ghost_densemat *, void *);
    ...
    SELECT_BLAS1_KERNEL(
        kernels, commonlocation, y->traits.compute_at, y->traits.storage, ret, y, x, a, b, z, c);

    return ret;
}

ghost_error ghost_vaxpbypcz(
    ghost_densemat *y, ghost_densemat *x, void *a, void *b, ghost_densemat *z, void *c)
{
    ...
    typedef ghost_error (*ghost_vaxpbypcz_kernel)(
        ghost_densemat *, ghost_densemat *, void *, void *, ghost_densemat *, void *);
...
ghost_error ghost_norm(void *norm, ghost_densemat *x, void *pow)
{
    ...
    SELECT_BLAS1_KERNEL(
        kernels, x->traits.location, x->traits.compute_at, x->traits.storage, ret, x, norm, pow);
...
ghost_error ghost_mult(ghost_densemat *y, ghost_densemat *x, void *a)
{
    ...
    if (ncolsX == 1) {
        kernels[GHOST_HOST_IDX][GHOST_RM_IDX] = &ghost_densemat_rm_mult1_selector;
        kernels[GHOST_HOST_IDX][GHOST_CM_IDX] = &ghost_densemat_cm_mult1_selector;
#ifdef GHOST_HAVE_CUDA
        kernels[GHOST_DEVICE_IDX][GHOST_RM_IDX] = &ghost_densemat_cu_rm_mult1;
        kernels[GHOST_DEVICE_IDX][GHOST_CM_IDX] = &ghost_densemat_cu_cm_mult1;
#endif
    } else {
