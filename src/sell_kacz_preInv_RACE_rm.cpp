...
#define LOOP_preInv(start,end,MT,VT) \
...
inline void KACZ_preInv_Kernel(int start, int end, void *args)
{
    ...
    MT *rnmInv_sq = (MT *)(kaczArg->mat->rowNormSqInv);
    MT omega = *((MT*)(kaczArg->omega));
    ghost_sparsemat *mat = kaczArg->mat;

    LOOP_preInv(start, end, MT, VT);
}
...
void ghost_kacz_preInv_RACE(ghost_densemat *b, ghost_sparsemat *mat, ghost_densemat *x, int iterations)
{
    ...
    int kaczId = ce->registerFunction(&KACZ_preInv_Kernel, argPtr);
