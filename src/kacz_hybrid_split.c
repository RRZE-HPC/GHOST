...
(mat->context->col_map->flags & GHOST_PERM_NO_DISTINCTION)?( (col_ptr[col_idx]<(mat->context->col_map->dim-mat->context->col_map->nhalo))?col_ptr[col_idx]:mat->context->col_map->loc_perm[col_ptr[col_idx]] ):col_ptr[col_idx]\
...
ghost_error find_zone_extrema(ghost_sparsemat *mat, int **extrema, ghost_lidx a, ghost_lidx b)
{
    ...
    for(int i=a; i<b; ++i) {
        ...
        ghost_lidx start_col =  mat->context->col_map->dimpad;
        ghost_lidx end_col   = 0;
        ghost_lidx idx = chunk_ptr[chunk]+rowinchunk;
        ...
        min_lower = MIN( min_lower, start_col);
        max_lower = MAX( max_lower, start_col);
        min_upper = MIN( min_upper, end_col );
        max_upper = MAX( max_upper, end_col );
    }
    ...
    for(int i=1; i<mat->context->kacz_setting.active_threads; ++i) {
        ...
        if(mat->context->kacz_setting.kacz_method == GHOST_KACZ_METHOD_BMC_one_sweep) {
            ...
            if( (zones[4*i+6] != zones[4*i+7]) && (zones[4*(i-1)+2] != zones[4*(i-1)+3]) && trans_min <= trans_max) {//col_ptr[row_ptr[zones[4*i+2]]] <= col_ptr[row_ptr[zones[4*i-1]]-1] ) {
                printf("trans_min = %"PRLIDX", trans_max = %"PRLIDX"\n",trans_min,trans_max);
...
*/
    for(int i=a; i<b; ++i){
        chunk = i/chunkheight;   //can avoid this by doing reminder loops
        rowinchunk = i%chunkheight;
        start_col =  mat->context->col_map->dimpad;
        end_col   = 0;
