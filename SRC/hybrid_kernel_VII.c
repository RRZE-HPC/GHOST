...
void hybrid_kernel_VII(int current_iteration, VECTOR_TYPE* res, LCRP_TYPE* lcrp, VECTOR_TYPE* invec){

   /*****************************************************************************
    ********                  Kernel ir -- cs -- wa -- ca                ********   
    ********          Kommunikation mittels MPI_ISend, MPI_IRecv         ********
    ********  pro to_PE Umkopieren und Senden in omp paralleler Region   ********
    ********        Korrektheit der Kommunikation ueber omp single       ********
    ********                erfordert MPI_THREAD_SERIALIZED              ********
    ****************************************************************************/
...
#pragma omp single
  ...
  spmvmKernAll( lcrp, invec, res, &asm_cyclecounter, &asm_cycles, &cycles4measurement,
   ...
   IF_DEBUG(1){
      ...
      printf("HyK_VII: PE %d: It %d: Absetzen IRecv [ms]               : %8.3f\n",
	    me, current_iteration, 1000*time_it_took);

      time_it_took = (1.0*cs_cycles)/clockfreq;
      printf("HyK_VII: PE %d: It %d: Umkopieren & Kommunikation [ms]   : %8.3f",
	    me, current_iteration, 1000*time_it_took);
      ...
      printf("HyK_VII: PE %d: It %d: MPI_Waitall [ms]                  : %8.3f\n",
	    me, current_iteration, 1000*time_it_took);

      time_it_took = (1.0*ca_cycles)/clockfreq;
      printf("HyK_VII: PE %d: It %d: SpMVM (alle Elemente) [ms]        : %8.3f",
	    me, current_iteration, 1000*time_it_took);
      ...
      printf("HyK_VII: PE %d: It %d: Res von Device [ms]               : %8.3f",
	    me, current_iteration, 1000*time_it_took);
      printf(" Datenvolumen: %6.3f MB (@%6.3f GB/s)\n", 8e-6*res->nRows, 
      ...
      printf("HyK_VII: PE %d: It %d: Kompletter Hybrid-kernel [ms]     : %8.3f\n", 
