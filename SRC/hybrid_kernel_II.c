...
#pragma omp parallel for schedule(runtime) private(j)
	...
	for (i=0 ; i<lcrp->nodes ; i++){
		if ( (lcrp->dues[i]>0) || (lcrp->wishes[i]>0) ){/* senden oder empfangen */
			...
			// permute remote portion of invec
		/*	IF_DEBUG(1) printf("##### PE%d: Permuting %d elements of rhs vector coming from PE%d with a permutation vector of length %d\n",me,lcrp->wishes[i],i,lcrp->rowPerm[i].nRows);
			permuteVector(&invec->val[hlpi],lcrp->rowPerm[i].val,lcrp->wishes[i]);
...
/*#ifdef CUDAKERNEL
#ifndef ELR
	IF_DEBUG(1) printf("##### PE%d: Permuting %d elements of rhs vector which has %d entries with a permutation vector of length %d\n",me,lcrp->lnRows[me],invec->nRows,lcrp->rowPerm[me].nRows);
	printf("##### PE%d: %f to %f, %f to %f\n",me,invec->val[0],invec->val[lcrp->lnRows[me]-1],invec->val[lcrp->lnRows[me]],invec->val[invec->nRows-1]);
	
	// permute local portion of invec
	permuteVector(invec->val,lcrp->rowPerm[me].val,lcrp->lnRows[me]);
#endif
#endif*/
	
	spmvmKernAll( lcrp, invec, res, &asm_cyclecounter, &asm_cycles, &cycles4measurement, &ca_cycles, &cp_in_cycles, &cp_res_cycles, &me);
