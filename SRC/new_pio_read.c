...
LCRP_TYPE* new_pio_read(char* testcase, int work_dist){
   ...
   IF_DEBUG(1){
      ...
      if (me==0) printf("Entering new_pio_read\n");
   }
...
// #define ROWPTR8
   ...
   if (me==0){
	 ...
	 for (i=1; i<lcrp->nodes; i++){
	    ...
	    lcrp->lfEnt[i] = gl_row_ptr[lcrp->lfRow[i]];
	 }
	 ...
	 for (i=0; i<lcrp->nodes; i++){
	    for (j=lcrp->lfEnt[i]; j<lcrp->lfEnt[i]+lcrp->lnEnts[i]; j++){
	       if (lcrp->col[j] >= lcrp->lfRow[i] && 
		     lcrp->col[j]<lcrp->lfRow[i]+lcrp->lnRows[i])
                  loc_count[i]++;
	    }
	 }
	 ...
	 for (i=0; i<lcrp->nodes; i++){
	  ...
	  for (i=0; i<lcrp->nodes-1; i++){
		...
		trial_count = 0;
		for (j=lcrp->lfEnt[i]; j<gl_row_ptr[lcrp->lfRow[i]+trial_rows]; j++){
		   if (lcrp->col[j] >= lcrp->lfRow[i] && lcrp->col[j]<lcrp->lfRow[i]+trial_rows)
		      trial_count++;
		}
		...
		prev_count = trial_count;
	     ...
	     if (lcrp->lfRow[i+1]>lcrp->nRows) printf("Exceeded matrix dimension\n");
	     lcrp->lfEnt[i+1] = gl_row_ptr[lcrp->lfRow[i+1]];
	     lcrp->lnEnts[i] = lcrp->lfEnt[i+1] - lcrp->lfEnt[i] ;
	  ...
	  for (j=lcrp->lfEnt[lcrp->nodes-1]; j<lcrp->nEnts; j++)
	     if (lcrp->col[j] >= lcrp->lfRow[lcrp->nodes-1]) loc_count[lcrp->nodes-1]++;
	  ...
	  IF_DEBUG(2) printf("insgesamt lokale Elemente: %d bzw pro PE: %d Gesamtanteil: %6.3f%%\n",
		hlpi, target_lnze, 100.0*hlpi/(1.0*lcrp->nEnts));
