...
void hybrid_kernel_XIV(int current_iteration, VECTOR_TYPE* res, LCRP_TYPE* lcrp, VECTOR_TYPE* invec){

   /*****************************************************************************
    ********             Kernel ir -- c|x -- lc|sw  -- nl|x              ********
    ...
    ********     - Reihenfolge der MPI_ISend gemaess Ringshift           ********
    ****************************************************************************/
   ...
   uint64 cp_cycles, ir_cycles, pr_cycles, lc_cycles, nl_cycles;
   ...
   IF_DEBUG(2){
	 ...
	 if (lcrp->wishes[from_PE]>0) printf
	    ("HyK_XIV: PE%d: erwarte %d Elemente von PE%d (tag=%d) und schreibe diese an Stelle %d in invec->val\n", 
...
#pragma omp single
	 ...
	 IF_DEBUG(2) printf("HyK_XIV: PE%d thread%d: von %d bis %d\n", 
...
#pragma omp single
   ...
   IF_DEBUG(1){
      ...
      printf("HyK_XIV: PE %d: It %d: Absetzen des Irecv [ms]           : %8.3f",
      ...
      printf("HyK_XIV: PE %d: It %d: SpMVM (nichtlokale Elemente) [ms] : %8.3f",
	    me, current_iteration, 1000*time_it_took);
      ...
      printf("HyK_XIV: PE %d: It %d: Kompletter Hybrid-kernel [ms]     : %8.3f\n", 
