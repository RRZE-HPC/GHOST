...
//   RegionId = likwid_markerRegisterRegion("Main");
   ...
   if (me == 0){
	 ...
	 case MPI_THREAD_MULTIPLE:
      ...
      if (io_format == 1){
         /* Parallel reading of matrix-file via MPI-IO
          * Nothing to do in the serial region */
      }
      else if (io_format == 2){
   ...
   */
   ...
   if( 0==me) {
     ...
     for(i=n_nodes-1; i>0; --i) {
      ...
      int k = lcrp->lnRows[i];
      ierr = MPI_Ssend(scatterBuf, k, MPI_DOUBLE, i, 1, MPI_COMM_WORLD);
