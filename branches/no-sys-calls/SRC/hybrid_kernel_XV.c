...
void hybrid_kernel_XV(int current_iteration, VECTOR_TYPE* res, LCRP_TYPE* lcrp, VECTOR_TYPE* invec){
   ...
   IF_DEBUG(2){
	 ...
	 if (lcrp->wishes[from_PE]>0) printf
	    ("HyK_XV: PE%d: erwarte %d Elemente von PE%d (tag=%d) und schreibe diese an Stelle %d in invec->val\n", 
...
#pragma omp single
	 ...
	 IF_DEBUG(2) printf("HyK_XV: PE%d thread%d: von %d bis %d\n", 
...
#pragma omp single
   ...
   IF_DEBUG(1){
      ...
      printf("HyK_XV: PE %d: It %d: Umkopieren [ms]                   : %8.3f",
      ...
      printf("HyK_XV: PE %d: It %d: SpMVM (nichtlokale Elemente) [ms] : %8.3f",
	    me, current_iteration, 1000*time_it_took);
      ...
      printf("HyK_XV: PE %d: It %d: Kompletter Hybrid-kernel [ms]     : %8.3f\n", 
